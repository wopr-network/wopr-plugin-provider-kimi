 ...
 */
function convertA2AToKimiMcpConfig(
	a2aServers: Record<string, A2AServerConfig>,
): Record<string, any> {
	...
	for (const [serverName, config] of Object.entries(a2aServers)) {
		...
		logger.info(
			`[kimi] Registered A2A server: ${serverName} with ${config.tools.length} tools`,
		);
	}
...
class KimiClient implements ModelClient {
	...
	async *query(opts: ModelQueryOptions): AsyncGenerator<unknown> {
		...
		if (opts.a2aServers && Object.keys(opts.a2aServers).length > 0) {
			...
			logger.info(
				`[kimi] A2A MCP servers configured: ${Object.keys(opts.a2aServers).join(", ")}`,
			);
		}
...
const manifest: PluginManifest = {
	...
	version: "1.6.0",
	description:
		"Moonshot AI Kimi Code CLI provider with OAuth and A2A/MCP support",
	author: "wopr-network",
	license: "MIT",
	...
	setup: [
		{
			...
			description:
				"Authenticate with Moonshot AI via `kimi auth login`. The CLI handles OAuth automatically.",
			optional: false,
		},
	],
	lifecycle: {
